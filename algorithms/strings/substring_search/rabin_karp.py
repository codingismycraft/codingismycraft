...
def rabin_karp_search(text, pattern):
    ...
    for k, c in enumerate(pattern):
        pattern_hash += _hash_for_char(c) * _SEED ** (len(pattern) - k - 1)
        rolling_hash += _hash_for_char(text[k]) * _SEED ** (len(pattern) - k - 1)

    for i in range(len(text) - len(pattern) + 1):
        if i > 0:
            rolling_hash -= _hash_for_char(text[i-1]) * _SEED ** (len(pattern)-1)
            rolling_hash *= _SEED
